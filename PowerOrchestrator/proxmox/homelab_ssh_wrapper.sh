...
case "$SSH_ORIGINAL_COMMAND" in
    ...
    "cpu=\$(grep 'cpu ' /proc/stat | awk '{usage=(\$2+\$4)*100/(\$2+\$4+\$5)} END {printf \"%.1f\", usage}'); ram=\$(free -m | awk 'NR==2 {printf \"%d:%d\", \$3, \$2}'); temp=\$(for z in /sys/class/thermal/thermal_zone*; do [ -f \"\$z/temp\" ] && read -r t < \"\$z/temp\" && read -r y < \"\$z/type\" && echo \"\$y:\$t\" || true; done | grep -iE 'pkg|cpu|soc|temp|tctl|tdie|edge|acpi' | head -n 1 | cut -d: -f2); [ -z \"\$temp\" ] && temp=\$(for h in /sys/class/hwmon/hwmon*; do [ -f \"\$h/name\" ] && read -r n < \"\$h/name\" && echo \"\$n\" | grep -qiE 'k10temp|coretemp|zenpower|cpu|soc' && [ -f \"\$h/temp1_input\" ] && read -r t < \"\$h/temp1_input\" && echo \"\$t\" && break || true; done); [ -z \"\$temp\" ] && temp=\$(for z in /sys/class/thermal/thermal_zone*; do [ -f \"\$z/temp\" ] && read -r t < \"\$z/temp\" && echo \"\$t\" && break; done); [ -z \"\$temp\" ] && temp=\$(for h in /sys/class/hwmon/hwmon*; do [ -f \"\$h/temp1_input\" ] && read -r t < \"\$h/temp1_input\" && echo \"\$t\" && break; done); [ -z \"\$temp\" ] && temp=0; temp=\$((temp/1000)); disk=\$(df -h / | awk 'NR==2 {print \$5}' | tr -d '%'); echo \"\$cpu|\$ram|\$temp|\$disk\"" | \
    "cpu=\$(grep 'cpu ' /proc/stat | awk '{usage=(\$2+\$4)*100/(\$2+\$4+\$5)} END {printf \"%.1f\", usage}'); ram=\$(free -m | awk 'NR==2 {printf \"%d:%d\", \$3, \$2}'); temp=\$(for z in /sys/class/thermal/thermal_zone*; do [ -f \"\$z/temp\" ] && read -r t < \"\$z/temp\" && read -r y < \"\$z/type\" && echo \"\$y:\$t\" || true; done | grep -iE 'pkg|cpu|soc|temp|tctl|tdie|edge|acpi' | head -n 1 | cut -d: -f2); [ -z \"\$temp\" ] && temp=\$(for h in /sys/class/hwmon/hwmon*; do [ -f \"\$h/name\" ] && read -r n < \"\$h/name\" && echo \"\$n\" | grep -qiE 'k10temp|coretemp|zenpower|cpu|soc' && [ -f \"\$h/temp1_input\" ] && read -r t < \"\$h/temp1_input\" && echo \"\$t\" && break || true; done); [ -z \"\$temp\" ] && temp=\$(for z in /sys/class/thermal/thermal_zone*; do [ -f \"\$z/temp\" ] && read -r t < \"\$z/temp\" && echo \"\$t\" && break; done); [ -z \"\$temp\" ] && temp=\$(for h in /sys/class/hwmon/hwmon*; do [ -f \"\$h/temp1_input\" ] && read -r t < \"\$h/temp1_input\" && echo \"\$t\" && break; done); [ -z \"\$temp\" ] && temp=0; temp=\$((temp/1000)); disk=\$(df -h / | awk 'NR==2 {print \$5}' | tr -d '%'); echo \"\$cpu|\$ram|\$temp|\$disk\"")
        cpu=$(grep 'cpu ' /proc/stat | awk '{usage=($2+$4)*100/($2+$4+$5)} END {printf "%.1f", usage}')
        ram=$(free -m | awk 'NR==2 {printf "%d:%d", $3, $2}')
        temp=$(for z in /sys/class/thermal/thermal_zone*; do [ -f "$z/temp" ] && read -r t < "$z/temp" && read -r y < "$z/type" && echo "$y:$t" || true; done | grep -iE 'pkg|cpu|soc|temp|tctl|tdie|edge|acpi' | head -n 1 | cut -d: -f2)
        if [ -z "$temp" ]; then
            temp=$(for h in /sys/class/hwmon/hwmon*; do [ -f "$h/name" ] && read -r n < "$h/name" && echo "$n" | grep -qiE 'k10temp|coretemp|zenpower|cpu|soc' && [ -f "$h/temp1_input" ] && read -r t < "$h/temp1_input" && echo "$t" && break || true; done)
        fi
        [ -z "$temp" ] && temp=$(for z in /sys/class/thermal/thermal_zone*; do [ -f "$z/temp" ] && read -r t < "$z/temp" && echo "$t" && break; done)
        [ -z "$temp" ] && temp=$(for h in /sys/class/hwmon/hwmon*; do [ -f "$h/temp1_input" ] && read -r t < "$h/temp1_input" && echo "$t" && break; done)
        [ -z "$temp" ] && temp=0
        temp=$((temp/1000))
